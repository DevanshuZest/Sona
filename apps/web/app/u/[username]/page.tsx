...
export default async function ProfilePage({ params }: { params: Promise<{ username: string }> }) {
    ...
    return <div><header className="mb-8"><h1 className="text-2xl font-semibold">{profile.display_name}</h1><p className="text-sm text-[#0F766E]">@{profile.username}</p>{profile.bio && <p className="mt-3 text-[#78716C]">{profile.bio}</p>}<p className="mt-2 text-xs text-[#78716C]">Joined {new Date(profile.created_at).toLocaleDateString()}</p><div className="mt-2 flex items-center gap-4 text-sm text-[#78716C]"><span><strong className="text-[#1C1917]">{followerCount ?? 0}</strong> followers</span><span><strong className="text-[#1C1917]">{followingCount ?? 0}</strong> following</span><FollowButton viewerId={viewer?.id ?? null} targetId={profile.id} /></div></header><div className="space-y-4">{posts?.length ? posts.map((post) => <article key={post.id} className="rounded-xl border border-[#E7E5E4] bg-white p-4"><p className="whitespace-pre-wrap">{post.content}</p><p className="mt-2 text-xs text-[#78716C]">{new Date(post.created_at).toLocaleString()}</p></article>) : <p className="text-sm text-[#78716C]">No posts yet.</p>}</div></div>;
}
